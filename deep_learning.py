...
"A trade-off between oscillatory modulation and single-unit computation." 
BioArxiv 2018

    Modifications by: Erik Peterson
    E-mail: erik.exists@gmail.com
    Date: March 29, 2019
    Institution: Carnegie Melon
    
Which was modified from the original code, for the simulations presented in
...
n_full_test = 1000  # number of examples to use for full tests  (every epoch)
...
record_plateau_times = True  # record plateau potential times for each neuron across training
record_voltages = True  # record voltages of neurons during training (huge arrays for long simulations!)
...
class Network:
    ...
    def init_weights(self):
        ...
        for m in xrange(self.M - 1, -1, -1):
            ...
            if use_weight_optimization:
                ...
                W_sm = (V_sm + (nu**2) * (N - N**2) * (W_avg**2) *
                        (V_avg**2) - 2 * N * nu * b_avg * V_avg * W_avg -
                        (b_avg**2)) / (N * (nu**2) * V_sm)
    ...
    def print_weights(self):
        ...
        for m in xrange(self.M - 1, -1, -1):
            ...
            if m != self.M - 1:
                print("\tY_avg: {0:.6f},\tY_sd: {1:.6f}.".format(
                    np.mean(self.Y[m]), np.std(self.Y[m])))
                if use_feedback_bias:
                    print("\tc_avg: {0:.6f},\tc_sd: {1:.6f}.".format(
                        np.mean(self.c[m]), np.std(self.c[m])))
    ...
    def init_layers(self):
        ...
        if self.M == 1:
            ...
            self.l.append(
                finalLayer(net=self, m=self.M - 1, f_input_size=self.n[-2]))
    ...
    def f_phase(self, x, t, training_num, training=False):
        ...
        if record_voltages and training:
            ...
            self.A_hists = [
                np.zeros((l_f_phase, self.l[m].size))
                for m in xrange(self.M - 1)
            ]
            self.B_hists = [
            ...
            if record_voltages and training:
                ...
                for m in xrange(self.M):
                    if m != self.M - 1:
                        ...
                        self.O_hists[m][time, :] = self.l[m].O
        ...
        if record_eigvals:
            ...
            self.J_betas.append(
                np.multiply(lambda_max * deriv_sigma(self.l[-1].average_C_f),
                            k_D * self.W[-1]))
            self.J_gammas.append(
                np.multiply(
                    deriv_sigma(
                        np.dot(self.Y[-2],
    ...
    def t_phase(self, x, t, training_num):
        ...
        if record_voltages:
            # initialize voltage arrays
            self.A_hists = [
                np.zeros((l_t_phase, self.l[m].size))
                for m in xrange(self.M - 1)
            ]
            self.B_hists = [
                np.zeros((l_t_phase, self.l[m].size)) for m in xrange(self.M)
            ]
            self.C_hists = [
                np.zeros((l_t_phase, self.l[m].size)) for m in xrange(self.M)
            ]
        ...
        if use_sparse_feedback and (use_symmetric_weights
                                    or update_feedback_weights):
            for m in xrange(self.M - 1):
    ...
    def train(self,
              ...
              n_training_examples,
              save_simulation,
              simulations_folder=default_simulations_folder,
              folder_name="",
              overwrite=False,
        ...
        if save_simulation:
            ...
            if folder_name == "":
                self.simulation_path = os.path.join(
                    simulations_folder, "{}.{}.{}-{}.{}".format(
                        sim_start_time.year, sim_start_time.month,
                        sim_start_time.day, sim_start_time.hour,
                        sim_start_time.minute))
            else:
                self.simulation_path = os.path.join(simulations_folder,
                                                    folder_name)
            ...
            if not os.path.exists(self.simulation_path):
                ...
                if overwrite == False:
                    print("Error: Simulation directory \"{}\" already exists.".
                          format(self.simulation_path))
                    return
                else:
            ...
            shutil.copyfile(filename,
                            os.path.join(self.simulation_path, filename))
            ...
            if not continuing:
                with open(
                        os.path.join(self.simulation_path, 'simulation.txt'),
                        'w') as simulation_file:
                    print(
                        "Simulation done on {}.{}.{}-{}.{}.".format(
                            sim_start_time.year, sim_start_time.month,
                            sim_start_time.day, sim_start_time.hour,
                            sim_start_time.minute),
                    ...
                    print(
                        "Start time: {}".format(sim_start_time),
                        file=simulation_file)
                    print(
                        "-----------------------------", file=simulation_file)
                ...
                with open(
                        os.path.join(self.simulation_path, 'simulation.json'),
                        'w') as simulation_file:
                    simulation_file.write(json.dumps(params))
            else:
                # load previously saved recording arrays
                self.prev_full_test_errs = np.load(
                    os.path.join(self.simulation_path, "full_test_errors.npy"))
                self.prev_quick_test_errs = np.load(
                    os.path.join(self.simulation_path,
                                 "quick_test_errors.npy"))

                if record_backprop_angle and not use_backprop:
                    self.prev_bp_angles = np.load(
                        os.path.join(self.simulation_path, "bp_angles.npy"))

                if record_loss:
                    self.prev_losses = np.load(
                        os.path.join(self.simulation_path,
                                     "final_layer_loss.npy"))

                if record_training_error:
                    self.prev_training_errors = np.load(
                        os.path.join(self.simulation_path,
                                     "training_errors.npy"))

                if record_training_labels:
                    self.prev_training_labels = np.load(
                        os.path.join(self.simulation_path,
                                     "training_labels.npy"))

                if record_plateau_times:
                    self.prev_plateau_times_full = [
                        np.load(
                            os.path.join(self.simulation_path,
                                         "plateau_times_{}.npy".format(m)))
                        for m in range(self.M)
                    ]

                if record_phase_times:
                    self.prev_phase_times = np.load(
                        os.path.join(self.simulation_path, "phase_times.npy"))

                if record_eigvals:
                    self.prev_max_jacobian_eigvals = np.load(
                        os.path.join(self.simulation_path,
                                     "max_jacobian_eigvals.npy"))
                    self.prev_max_weight_eigvals = np.load(
                        os.path.join(self.simulation_path,
                                     "max_weight_eigvals.npy"))
                    if record_matrices:
                        self.prev_jacobian_prod_matrices = np.load(
                            os.path.join(self.simulation_path,
                                         "jacobian_prod_matrices.npy"))
                        self.prev_weight_prod_matrices = np.load(
                            os.path.join(self.simulation_path,
                                         "weight_prod_matrices.npy"))
        ...
        if record_plateau_times:
            self.plateau_times_full = [
                np.zeros((n_epochs * 2 * n_training_examples, self.n[m]))
                for m in range(self.M)
            ]
        ...
        if record_phase_times:
            ...
            for i in xrange(1, 2 * n_epochs * n_training_examples):
                if i % 2 == 0:
                    self.phase_times[i] = self.phase_times[i - 1] + l_f_phases[
                        int(i / 2)]
                else:
        ...
        if record_eigvals:
            ...
            self.max_jacobian_eigvals = np.zeros(
            ...
            if not continuing:
                ...
                if record_matrices:
                    self.weight_prod_matrices = np.zeros(
                        (n_epochs * n_training_examples + 1, self.n[-1],
                         self.n[-1]))
            else:
                self.max_weight_eigvals = np.zeros(
                    n_epochs * n_training_examples)
                if record_matrices:
                    self.weight_prod_matrices = np.zeros(
                        (n_epochs * n_training_examples, self.n[-1],
                         self.n[-1]))
        ...
        for k in xrange(n_epochs):
            ...
            self.x_train, self.t_train = shuffle_arrays(
            ...
            if use_rand_plateau_times:
                self.plateau_times_f = [
                    np.zeros((n_training_examples, self.n[m])) +
                    l_f_phases[k * n_training_examples:
            ...
            if use_rand_plateau_times:
                self.plateau_times_t = [
                    np.zeros((n_training_examples, self.n[m])) +
                    l_t_phases[k * n_training_examples:
                               (k + 1) * n_training_examples, np.newaxis] - 1 -
                    np.minimum(
                        np.abs(
                            np.random.normal(
            ...
            for n in xrange(n_training_examples):
                ...
                if record_plateau_times:
                    total_time_to_forward_phase = np.sum(
                        l_f_phases[:k * n_training_examples + n]) + np.sum(
                            l_t_phases[:k * n_training_examples + n])
                    total_time_to_target_phase = np.sum(
                        l_f_phases[:k * n_training_examples + n + 1]) + np.sum(
                            l_t_phases[:k * n_training_examples + n])
                    for m in range(self.M):
                        self.plateau_times_full[m][
                            k * n_training_examples + 2 *
                            n] = total_time_to_forward_phase + self.plateau_times_f[
                                m][n]
                        self.plateau_times_full[m][
                            k * n_training_examples + 2 * n +
                            1] = total_time_to_target_phase + self.plateau_times_t[
                                m][n]

                # print every 100 examples
                if (n + 1) % 100 == 0:
                    sys.stdout.write(
                        "\x1b[2K\rEpoch {0}, example {1}/{2}.".format(
                            self.current_epoch + 1, n + 1,
                            n_training_examples))
                    sys.stdout.flush()
                ...
                if record_training_error:
                    sel_num = np.argmax(
                        np.mean(
                            self.l[-1].average_C_f.reshape(
                                -1, self.n_neurons_per_category),
                            axis=-1))
                ...
                self.t_phase(
                    self.x, self.t.repeat(self.n_neurons_per_category, axis=0),
                    n)
                ...
                if record_training_labels:
                    self.training_labels[k * n_training_examples + n] = np.dot(
                        np.arange(10), self.t)
                ...
                if record_eigvals:
                    ...
                    U = np.mean(
                        np.array([
                            np.dot(self.J_betas[i], self.J_gammas[i])
                            for i in range(len(self.J_betas))
                        ]),
                        axis=0)  # mean of product of last 100 Jacobians

                    p = np.dot((I - U).T, I - U)
                    if record_matrices:
                        self.jacobian_prod_matrices[k * n_training_examples +
                                                    n] = U
                    ...
                    if not continuing:
                        if record_matrices:
                            self.weight_prod_matrices[k * n_training_examples +
                                                      n + 1] = U
                        self.max_weight_eigvals[k * n_training_examples + n +
                                                1] = np.amax(
                                                    np.linalg.eigvals(p))
                    else:
                        if record_matrices:
                ...
                if record_backprop_angle and not use_backprop:
                    ...
                    if self.M > 1:
                        bp_angle = np.arccos(
                            np.sum(self.l[0].delta_b_bp *
                                   self.l[0].delta_b_full) /
                            (np.linalg.norm(self.l[0].delta_b_bp) * np.linalg.
                             norm(self.l[0].delta_b_full.T))) * 180.0 / np.pi
                ...
                if plot_eigvals and record_eigvals and (n + 1) % 100 == 0:
                    ...
                    if record_matrices:
                        A = np.mean(
                            np.array([
                                self.jacobian_prod_matrices[
                                    k * n_training_examples + n -
                                    99:k * n_training_examples + n + 1][i]
                                for i in max_inds
                            ][:-10]),
                            axis=0)
                        im_plot.set_data(A)
                    ...
                    if record_loss:
                        ...
                        ax2.set_ylim(
                            np.amin(self.losses[:k * n_training_examples + n])
                            - 1e-6,
                            np.amax(self.losses[:k * n_training_examples + n])
                            + 1e-6)
                    ...
                    ax3.set_ylim(
                        np.amin(self.
                                max_jacobian_eigvals[:k * n_training_examples +
                                                     n]) - 1e-6,
                        np.amax(self.
                                max_jacobian_eigvals[:k * n_training_examples +
                                                     n]) + 1e-6)
                ...
                if (n + 1) % 1000 == 0:
                    if n != n_training_examples - 1:
                        ...
                        sys.stdout.write(
                            "\x1b[2K\rEpoch {0}, example {1}/{2}. QE: {3:05.2f}%. "
                            .format(self.current_epoch + 1, n + 1,
                                    n_training_examples, test_err))

                        if not continuing:
                            self.quick_test_errs[(k + 1) * int(
                                n_training_examples / 1000)] = test_err
                        else:
                            self.quick_test_errs[
                                (k + 1) * int(n_training_examples / 1000) -
                                1] = test_err
                        ...
                        if save_simulation:
                            ...
                            if not continuing:
                                ...
                                if record_plateau_times:
                                    plateau_times_full = [
                                        self.plateau_times_full[m]
                                        [:(k + 1) * 2 * n_training_examples]
                                        for m in range(self.M)
                                    ]

                                if record_training_error:
                                    training_errors = self.training_errors[:k +
                                                                           1]
                                ...
                                quick_test_errs = np.concatenate([
                                    self.prev_quick_test_errs,
                                    self.quick_test_errs[:(
                                        k +
                                        1) * int(n_training_examples / 1000)]
                                ],
                                                                 axis=0)
                                if n == n_training_examples - 1:
                                    full_test_errs = np.concatenate([
                                        self.prev_full_test_errs,
                                        self.full_test_errs[:k + 1]
                                    ],
                                                                    axis=0)
                                ...
                                if record_plateau_times:
                                    plateau_times_full = [
                                        np.concatenate([
                                            self.prev_plateau_times_full[m],
                                            self.plateau_times_full[m]
                                            [:(k + 1) * 2 *
                                             n_training_examples]
                                        ]) for m in range(self.M)
                                ...
                                if record_training_error:
                                    training_errors = np.concatenate([
                                        self.prev_training_errors,
                                        self.training_errors[:k + 1]
                                    ],
                                                                     axis=0)
                                ...
                                if record_eigvals:
                                    max_jacobian_eigvals = np.concatenate(
                                        [
                                            self.prev_max_jacobian_eigvals,
                                            self.max_jacobian_eigvals[:(
                                                k + 1) * n_training_examples]
                                        ],
                                        axis=0)
                                    max_weight_eigvals = np.concatenate([
                                        self.prev_max_weight_eigvals,
                                        self.max_weight_eigvals[:(
                                            k + 1) * n_training_examples]
                                    ],
                                                                        axis=0)
                                    if record_matrices:
                                        jacobian_prod_matrices = np.concatenate(
                                            [
                                                self.
                                                prev_jacobian_prod_matrices,
                                                self.jacobian_prod_matrices[:(
                                                    k +
                                                    1) * n_training_examples]
                                            ],
                                            axis=0)
                                        weight_prod_matrices = np.concatenate(
                                            [
                                                self.prev_weight_prod_matrices,
                                                self.weight_prod_matrices[:(
                                                    k +
                                                    1) * n_training_examples]
                                            ],
                                            axis=0)
                            ...
                            if n == n_training_examples - 1:
                                ...
                                np.save(
                                    os.path.join(self.simulation_path,
                                                 "full_test_errors.npy"),
                                    full_test_errs)

                                # save weights
                                self.save_weights(
                                    self.simulation_path,
                                    prefix="epoch_{}_".format(
                                ...
                                if record_matrices:
                                    np.save(
                                        os.path.join(
                                            self.simulation_path,
                                            "jacobian_prod_matrices.npy"),
                                        jacobian_prod_matrices)
                                    np.save(
                                        os.path.join(
                                            self.simulation_path,
                                            "weight_prod_matrices.npy"),
                                        weight_prod_matrices)
                    ...
                    if record_eigvals:
                        ...
                        print(
                            "Min max Jacobian eigval: {:.4f}. ".format(
                                np.amin(self.max_jacobian_eigvals[max(
                                    0, k * n_training_examples + n -
                                    999):k * n_training_examples + n + 1])),
                            end="")

                        # print the number of max eigenvalues of (I - J_g*J_f).T * (I - J_g*J_f) from the last 1000 examples that were smaller than 1
                        print(
                            "# max eigvals < 1: {}. ".format(
                                np.sum(self.max_jacobian_eigvals[max(
                                    0, k * n_training_examples + n - 999
                                ):k * n_training_examples + n + 1] < 1)),
                            end="")
        ...
        if save_simulation:
            with open(
                    os.path.join(self.simulation_path, 'simulation.txt'),
                    'a') as simulation_file:
                sim_end_time = datetime.datetime.now()
                print("-----------------------------", file=simulation_file)
                print(
                    "End time: {}".format(sim_end_time), file=simulation_file)
    ...
    def test_weights(self, n_test=n_quick_test):
        ...
        for n in xrange(n_test):
            ...
            if (n + 1) % 100 == 0:
                sys.stdout.write(
                    "\x1b[2K\rTesting example {0}/{1}. E: {2:05.2f}%.".format(
                        n + 1, n_test,
                        (1.0 - float(num_correct) / (n + 1)) * 100.0))
                sys.stdout.flush()
    ...
    def save_weights(self, path, prefix=""):
        ...
        for m in xrange(self.M):
            np.save(
                os.path.join(path, prefix + "W_{}.npy".format(m)), self.W[m])
            np.save(
                os.path.join(path, prefix + "b_{}.npy".format(m)), self.b[m])
            if m != self.M - 1:
                np.save(
                    os.path.join(path, prefix + "Y_{}.npy".format(m)),
                    self.Y[m])
                if use_feedback_bias:
                    np.save(
                        os.path.join(path, prefix + "c_{}.npy".format(m)),
                        self.c[m])
    ...
    def load_weights(self, path, prefix=""):
        ...
        print("Loading weights from \"{}\" with prefix \"{}\".".format(
            path, prefix))
        print("--------------------------------")

        for m in xrange(self.M):
            self.W[m] = np.load(
                os.path.join(path, prefix + "W_{}.npy".format(m)))
            self.b[m] = np.load(
                os.path.join(path, prefix + "b_{}.npy".format(m)))
            if m != self.M - 1:
                self.Y[m] = np.load(
                    os.path.join(path, prefix + "Y_{}.npy".format(m)))
                if use_feedback_bias:
...
class hiddenLayer(Layer):
    ...
    def create_integration_vars(self):
        ...
        self.O_hist = np.zeros((self.size, integration_time))
    ...
    def update_W(self):
        ...
        if not use_backprop:
            self.E = (self.alpha_t - self.alpha_f
                      ) * -k_B * lambda_max * deriv_sigma(self.average_C_f)

            if record_backprop_angle and not use_backprop and calc_E_bp:
                self.E_bp = (np.dot(self.net.W[self.m + 1].T,
                                    self.net.l[self.m + 1].E_bp) * k_B *
                             lambda_max * deriv_sigma(self.average_C_f))
        else:
            self.E_bp = (
                np.dot(self.net.W[self.m + 1].T, self.net.l[self.m + 1].E_bp) *
                k_B * lambda_max * deriv_sigma(self.average_C_f))
            self.E = self.E_bp
        ...
        self.net.W[self.
                   m] += -self.net.f_etas[self.m] * P_hidden * self.delta_W

        self.delta_b = self.E
        self.net.b[self.
                   m] += -self.net.f_etas[self.m] * P_hidden * self.delta_b
    ...
    def update_Y(self):
        ...
        E_inv = (lambda_max * sigma(self.average_C_f) -
                 self.alpha_f) * -deriv_sigma(self.average_A_f)
    ...
    def update_A(self, b_input):
        ...
        self.PSP_A_hist[:, self.integration_counter %
                        integration_time] = self.PSP_A[:, 0]

        if use_feedback_bias:
            self.A = np.dot(self.net.Y[self.m],
                            self.PSP_A) + self.net.c[self.m]
        else:
            self.A = np.dot(self.net.Y[self.m], self.PSP_A)
        self.A_hist[:, self.integration_counter %
                    integration_time] = self.A[:, 0]
    ...
    def update_C(self):
        ...
        if use_conductances:
            ...
            if use_apical_conductance:
                self.C_dot = -g_L * self.C + g_B * (self.B - self.C) + g_A * (
                    self.A - self.C)
            else:
        ...
        self.C_hist[:, self.integration_counter %
                    integration_time] = self.C[:, 0]
        self.lambda_C = lambda_max * sigma(self.C)
        self.lambda_C_hist[:, self.integration_counter %
                           integration_time] = self.lambda_C[:, 0]
        self.O_hist[:, self.integration_counter % integration_time] = self.O
    ...
    def out_f(self, f_input, b_input):
        ...
        self.integration_counter = (
            self.integration_counter + 1) % integration_time
    ...
    def plateau_f(self, plateau_indices):
        ...
        self.average_A_f[plateau_indices] = np.mean(
            self.A_hist[plateau_indices], axis=-1)[:, np.newaxis]

        # calculate apical calcium spike nonlinearity
        self.alpha_f[plateau_indices] = sigma(
            self.average_A_f[plateau_indices])
    ...
    def plateau_t(self, plateau_indices):
        ...
        # calculate average apical potentials for neurons undergoing plateau potentials
        self.average_A_t[plateau_indices] = np.mean(
            self.A_hist[plateau_indices], axis=-1)[:, np.newaxis]

        # calculate apical calcium spike nonlinearity
        self.alpha_t[plateau_indices] = sigma(
            self.average_A_t[plateau_indices])
    ...
    def calc_averages(self, phase):
        ...
        if phase == "forward":
            ...
            if update_feedback_weights:
                self.average_PSP_A_f = np.mean(
                    self.PSP_A_hist, axis=-1)[:, np.newaxis]
        elif phase == "target":
...
class finalLayer(Layer):
    ...
    def update_W(self):
        ...
        self.E = (
            self.average_lambda_C_t - lambda_max * sigma(self.average_C_f)
        ) * -k_D * lambda_max * deriv_sigma(self.average_C_f)
...
def load_simulation(latest_epoch,
                    folder_name,
                    simulations_folder=default_simulations_folder):
    '''
    ...
    # load parameters
    with open(os.path.join(simulation_path, 'simulation.json'),
              'r') as simulation_file:
    ...
    kappas = np.flipud(
        get_kappas(mem))[:, np.newaxis]  # re-initialize kappas array
...
def load_MNIST(n_valid=0):
    ...
    try:
        ...
        print(
            "Could not find MNIST .npy files in the current directory.\nLooking for original binary files..."
        )
        try:
            if n_valid != 0:
...
def get_MNIST(n_valid=0):
    ...
    if (os.path.isfile("train-images.idx3-ubyte")
            and os.path.isfile("train-labels.idx1-ubyte")
            and os.path.isfile("t10k-images.idx3-ubyte")
            and os.path.isfile("t10k-labels.idx1-ubyte")):
...
def plot_weights(W_list, save_dir=None, suffix=None, normalize=False):
    ...
    for m in xrange(M):
        ...
        for k in xrange(n[m]):
            ...
            if normalize:
                heatmap = axes[m][k].imshow(
                    W_list[m][k].reshape(img_Bims).T,
                ...
                heatmap = axes[m][k].imshow(
                    W_list[m][k].reshape(img_Bims).T,
                    interpolation="nearest",
                    vmin=W_avgs[m] - 3.465 * W_sds[m],
                    vmax=W_avgs[m] + 3.465 * W_sds[m],
                    cmap=weight_cmap)
            axes[m][k].set_xticklabels([])
            axes[m][k].set_yticklabels([])

            axes[m][k].tick_params(
